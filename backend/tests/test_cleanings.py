...
class TestGetCleaning:
    ...
    """
    In the test below, we're parametrizing the test and passing in multiple invalid ids. 
    Each one is coupled with the status code we expect to see when 
    we send that id to our GET route.
    """

    @pytest.mark.parametrize(
        "id, status_code",
        (
            (500, 404),  # id 50 returns 404 as status_code
            (-1, 404),  # id -1 returns 404 as status_code
            (None, 422),  # id None returns 422 as status_code
        ),
    )
