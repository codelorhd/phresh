...
class TestUserLogin:
    async def test_user_can_login_successfully_and_receives_valid_token(
        ...
        # we're updating the content-type header to accept form data
        # instead of JSON. This matches the same OAuth2 specs
        client.headers["content-type"] = "application/x-www-form-urlencoded"
        login_data = {
            "username": test_user.email,
            "password": "testuserpassword",  # insert user's plaintext password
        }
    ...
    async def test_user_with_wrong_creds_doesnt_receive_token(
        ...
        user_data["password"] = "testuserpassword"  # insert user's plaintext password
...
class TestUserRegistration:
    ...
    async def test_users_can_register_successfully(
        ...
        user_repo = UsersRepository(db)
        new_user = {
            "email": "solomonaboyeji@gmail.com",
            "username": "solomonaboyeji",
            "password": "admin324",
        }
        ...
        # send post request to create user and ensure it is successfuly
        res = await client.post(
            app.url_path_for("users:register-new-user"), json={"new_user": new_user}
        )
        assert res.status_code == HTTP_201_CREATED
        ...
        # We exclude the password and salt attributes from the user record queried from our
        # database, as those should not be made available by any request that returns a user.
    ...
    @pytest.mark.parametrize(
        ...
        (
            ("email", "solomonaboyeji@gmail.com", 400),
            ("username", "solomonaboyeji", 400),
            ("email", "invalid_email@gmail.com@mail.com", 422),
            ("password", "short", 422),
